	      ...
	      <div class="caption">
	        ...
	        <p>
	        	<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=one&id=${list.id}" 
	        		class="btn btn-primary" role="button" style="font-size: 10px;margin-bottom:2px;"> 查 看  详 情 </a> 
	        	<a href="#" class="btn btn-default" role="button" style="font-size: 10px;" onclick="jump(${list.id})">加入购物车</a>
	        </p>
	      </div>
	    </div>
	  </div>
	...
	<div class="col-md-12">
		...
		<nav aria-label="Page navigation" style="float:right;margin-right:1%; margin-top:-20px;">
		  <ul class="pagination">
		    ...
		    <c:if test="${PageInfo.pageNumber > 1 }">
		    	<li>
				    <a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=${PageInfo.pageNumber-1}">
				        <span>&laquo;</span>
				    </a>
			    </li>
		    </c:if>
		    ...
		    <c:if test="${PageInfo.total > 5 }">
	    		...
	    		<c:if test="${PageInfo.pageNumber == 1}">
		    		...
		    		<li>
		    			<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=2" > 2 </a>
		    		</li>
		    		<li>
		    			<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=3" > 3 </a>
		    		</li>
		    		<li>
	    		...
	    		<c:if test="${PageInfo.pageNumber > 1}">
		    		<li>
		    			<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=1" > 1 </a>
		    		</li>
	    		</c:if>
	    		...
	    		<c:if test="${PageInfo.pageNumber == PageInfo.total}">
	    			...
	    			<li>
		    			<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=${PageInfo.pageNumber -2}"> 
	    		...
	    		<c:if test="${PageInfo.pageNumber < PageInfo.total}">
		    		<li>
		    			<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=${PageInfo.total}"> 
		    				${PageInfo.total} 
		    			</a>
		    		</li>
	    		</c:if>
		    ...
		    <c:if test="${PageInfo.total <= 5 }">
		    	<c:forEach begin="1" end="${PageInfo.total }" step="1" var="pNumber" varStatus="id">
		    		...
		    		<c:if test="${PageInfo.pageNumber != id.index }">
			    		<li>
			    			<a href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&pageNumber=${pNumber}"> 
			    				${pNumber} 
			    			</a>
			    		</li>
		    		</c:if>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#search").click(function(){
				...
				//alert(value);
				location.href="${pageContext.request.contextPath}/SelectGoodsServlet?flag=category&categoryId="+value;
			});
